...
export default function PrecoJustoRNCard({ insight, compact = false }: Props) {
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-4 text-2xl font-semibold text-slate-950 dark:text-white">Preco Justo RN</h3>
      ...
      {hasData && (
        <p className={`mt-3 text-sm font-extrabold ${diffColor}`}>
          {diffLabel}
        </p>
      )}
...
function Stat({ label, value, featured = false }: { label: string; value: string; featured?: boolean }) {
  return (
    <div className="rounded-xl border border-sand-200 bg-sand-50 px-3 py-3 dark:border-slate-800 dark:bg-slate-950/40">
      ...
      <p className={`mt-1.5 font-extrabold text-slate-950 dark:text-white ${featured ? 'text-xl' : 'text-base'}`}>{value}</p>
    </div>
  );
}
