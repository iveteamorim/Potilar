-- Tour virtual 3D por anuncio (Matterport, Kuula, etc.)
-- Execute no Supabase SQL Editor (projeto Potilar).

alter table public.listings add column if not exists tour_url text;

comment on column public.listings.tour_url is 'Optional embeddable 3D virtual tour URL (Matterport, Kuula, CloudPano, etc.).';
